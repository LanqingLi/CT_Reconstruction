...
DATA_FOLDER = '/media/tx-eva-cc/data/Infervision/test_gt/dcm/'
    # '/media/tx-eva-cc/data/2018_03_26_WuHanTongJi/'
FNAME = 'BJYY00191495T125'
    # 'LDLungSS0'
        ...
        for fname in os.listdir(data_folder):
            if os.path.isfile(os.path.join(data_folder, fname)):
                ...
                if window_shift:
                    ...
                    img_array[np_array < min_hu_new] = min_hu_new
                    img_array[(np_array <= max_hu) & (np_array >= min_hu)] = \
                        ((np_array[(np_array <= max_hu) & (np_array >= min_hu)] - window_center) / window_width + 0.5) * 255
                    img_array[np_array > max_hu] = 255
                    plt.imsave(os.path.join(path, fname[:-3] + 'png'), normalized(np.flip(img.transpose(), axis=0), True),
                               cmap='gray')
    ...
    for fname in os.listdir(os.path.join(DATA_FOLDER, FNAME)):
        if os.path.isfile(os.path.join(DATA_FOLDER, FNAME, fname)):
            ...
            if window_shift:
                ...
                img_array[np_array < min_hu_new] = min_hu_new
                img_array[(np_array <= max_hu) & (np_array >= min_hu)] = \
                    ((np_array[(np_array <= max_hu) & (np_array >= min_hu)] - window_center) / window_width + 0.5) * 255.0
